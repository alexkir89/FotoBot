...
- `/add_product CODE|Название|class_id|our(1/0)` — admin (из `ADMIN_IDS`; либо по умолчанию, если список пуст и включён `DEFAULT_ADMIN_IF_EMPTY`)
...
1) Заполните `.env`: `BOT_TOKEN`, `DB_PASSWORD`, `ADMIN_IDS`.
   - Если `ADMIN_IDS` пустой и `DEFAULT_ADMIN_IF_EMPTY=1`, бот даёт admin-доступ по умолчанию (для первичного запуска).
